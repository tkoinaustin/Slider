...
class GameModel {
  ...
  init(_ gameBoard: [[Int]]) {
    ...
    gameBoardBlocks = GameModelInitialization().initBlocks(grid: gameBoard)
  ...
  func setGameplayForDirection(_ direction: Direction, _ index: Int) -> Bool {
    ...
    start.text =  GameModelUtility.showBoard(board: .moveZeroSpaces, grid: gameBoard)
    finish.text = GameModelUtility.showBoard(board: .moveOneSpace, grid: oneMoveBoard)
    twoMove.text = GameModelUtility.showBoard(board: .moveTwoSpaces, grid: twoMoveBoard)
